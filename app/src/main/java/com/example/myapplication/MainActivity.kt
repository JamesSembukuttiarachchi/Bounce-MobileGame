...
class MainActivity : AppCompatActivity() {
    ...
    private var score = 0
    private var highestScore = 0 // Variable to store the highest score
    ...
    private fun checkCollision() {
        ...
        if (ballY + ball.height >= paddle.y && ballY + ball.height <= paddle.y + paddle.height
            ...
            updateHighestScore() // Update highest score when the score increases
        }
    ...
    private fun resetGame() {
        score = 0
        scoreText.text = "Score: $score"
        brickContainer.removeAllViews() // Clear the brick container
        initializeBricks() // Reinitialize bricks for a new game
    }
